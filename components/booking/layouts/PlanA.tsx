...
export default function PlanA({ getTableStyle, handleTableClick }: any) {
  return (
    <g id="party-floor-1">
      ...
      {FLOOR_1_TABLES.map((t) => (
        <g key={t.id} className="select-none cursor-pointer" onClick={() => handleTableClick(t.id)}>
          ...
          <text x={t.type === 'circle' ? t.cx : t.x! + (t.w! / 2)} y={t.type === 'circle' ? t.cy! + 3 : t.y! + (t.h! / 2) + 3} fill="#ffffff" fontSize="8" fontWeight="bold" textAnchor="middle" fontFamily="sans-serif" className="pointer-events-none">
            {t.label}
          </text>
        </g>
      ))}
