...
export default function FloorPlan({ selectedTable, setSelectedTable, dayTables = {} }: FloorPlanProps) {
  ...
  return (
    <div className="w-full h-[450px] relative bg-slate-950 rounded-2xl overflow-hidden select-none border border-slate-800/80 flex items-center justify-center">
      <TransformWrapper initialScale={1} minScale={0.8} maxScale={4} centerOnInit={true} wheel={{ disabled: false }} limitToBounds={false}>
        {({ zoomIn, zoomOut, resetTransform }) => (
          <>
            <div className="absolute top-3 right-3 z-30 flex flex-col gap-1 bg-black/60 border border-slate-800 p-1 rounded-xl backdrop-blur-md">
              <button type="button" onClick={() => zoomIn()} className="w-7 h-7 flex items-center justify-center bg-slate-900 hover:bg-slate-800 text-white rounded-lg font-bold text-sm border border-slate-700/40 transition-all active:scale-90">+</button>
